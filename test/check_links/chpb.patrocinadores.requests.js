...
describe(`Should check broken links on Patrocinadores page `, async function () {
  ...
  it(`Collect Patrocinadores Page links 'href' attributes and perform HTTP 'GET' requests on them validating the status code is 200`, async function () {
  ...
  it(`Collect Patrocinadores Page images 'src' attributes and perform HTTP 'GET' requests on them validating the status code is 200`, async function () {
    const footer = await PatrocinadoresPage.footer
    await expect(footer).toBeDisplayed()
    await browser.pause(2000)
