...
describe(`Should check broken links on Instalaciones page `, async function () {
  ...
  it(`Collect Instalaciones Page links 'href' attributes and perform HTTP 'GET' requests on them validating the status code is 200`, async function () {
  ...
  it(`Collect Instalaciones Page images 'src' attributes and perform HTTP 'GET' requests on them validating the status code is 200`, async function () {
    const footer = await InstalacionesPage.footer
    await expect(footer).toBeDisplayed()
    await browser.pause(2000)
