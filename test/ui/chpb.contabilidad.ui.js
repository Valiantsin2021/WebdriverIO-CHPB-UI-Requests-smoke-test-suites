...
const { bannersLinks, title } = require('../../utils/constants.contabilidad')
describe(`Should open Contabilidad Page and check`, async function () {
  ...
  it(`Cookies modal is displayed`, async function () {
    ...
    await expect(ContabilidadPage.cookiesModal).toExist()
    await ContabilidadPage.cookiesAcceptBtn.waitForDisplayed()
  })
  it(`Contabilidad Page header is ${clubHeader}`, async function () {
    console.info(`Check Contabilidad Page header is ${clubHeader}`)
    await ContabilidadPage.acceptCookies()
    const headerText = await ContabilidadPage.header.waitAndGetText()
    expectChai(headerText).to.eq(clubHeader)
  })
  it(`Contabilidad Page title is displayed`, async function () {
    console.info(`Contabilidad Page title is displayed`)
    const title = await ContabilidadPage.title
    await expect(title).toBeDisplayed()
  })
  it(`Contabilidad Page title is ${title}`, async function () {
    console.info(`Check Contabilidad Page title is '${title}'`)
    const pageTitle = await ContabilidadPage.title.waitAndGetText()
  ...
  it(`Home page club logotype is displayed`, async function () {
    ...
    const logotype = await ContabilidadPage.logotype
  ...
  it(`Contabilidad Page main image content is displayed`, async function () {
    console.info(`Contabilidad Page main image content is displayed`)
    await expect(ContabilidadPage.mainImg).toExist()
    await expect(ContabilidadPage.mainImg).toBeDisplayed()
  })
  it(`Top menu elements are displayed`, async function () {
    console.info(`Top menu elements are displayed`)
    await ContabilidadPage.header.waitForDisplayed()
    const topMenu = await ContabilidadPage.topMenuDropdownElements
  ...
  it(`Top menu number of elements`, async function () {
    ...
    const topMenuLength = await ContabilidadPage.topMenuDropdownElements
  ...
  it(`Top menu inner elements are displayed`, async function () {
    ...
    await showInnerMenu(ContabilidadPage.topMenuContainer)
    const topMenuInner = await ContabilidadPage.topMenuInnerElements
  ...
  it(`Top menu number of inner elements`, async function () {
    ...
    const topMenuInnerLength = await ContabilidadPage.topMenuInnerElements
  ...
  it(`Top social links are displayed`, async function () {
    ...
    await hideInnerMenu(ContabilidadPage.topMenuContainer)
    await ContabilidadPage.header.waitForDisplayed()
    const socialMedia = await ContabilidadPage.socialMediaLinksTop
  ...
  it(`Top social links number of elements`, async function () {
    ...
    const socialMediaLength = await ContabilidadPage.socialMediaLinksTop
  ...
  it(`Top social links href attribute against '${socialMediaLinks.join(
    ...
    const socialLinks = await ContabilidadPage.socialMediaLinksTop.map(el =>
  ...
  it(`Image banners on the Home page are displayed`, async function () {
    ...
    const banners = await ContabilidadPage.imageBanners
  ...
  it(`Image banners on the Home page href attribute against '${bannersLinks.join(
    ...
    const bannerHrefs = await ContabilidadPage.bannersLinks.map(el =>
      el.getAttribute('href')
    )
  ...
  footerLinksText.forEach((el, i) => {
    it(`Footer links text corresponds '${el}'`, async function () {
      ...
      const footerLinks = await ContabilidadPage.footerLinks.slice(0, -1)
  ...
  sponsorCategoriesText.forEach((el, i) => {
    it(`Sponsor categories each element\'s text corresponds '${el}'`, async function () {
      ...
      const sponsorCategories = await ContabilidadPage.sponsorsCategories
  ...
  it(`Sponsors list number of elements is ${sponsorsNumber}`, async function () {
    ...
    const sponsors = await ContabilidadPage.sponsorsList
